...
const CartPage = () => {
    ...
    return (
        <div className={back ? 'fixed right-0 top-0 w-full h-screen bg-black/70' : '' }>
            <div className={
                back ? 'fixed top-0 right-0 w-full sm:w-[45%] lg:w-[33%] h-screen bg-slate-100 dark:bg-[#121212] drop-shadow-md flex flex-col z-[101] ease-in duration-500'
                : 'fixed left-[-100%] top-0 p-4 ease-in duration-500'}>
                <div className='w-full flex items-center justify-between p-4 cursor-pointer'>
                    <MdOutlineKeyboardBackspace size={22} onClick={handleBack}/>
                    ...
                    {cart.cartItems.length === 0 ? (
                        <div className='flex items-center justify-center flex-col py-16 p-5'>
                            ...
                            <div>
                                <Link to='/shoppage' className='cursor-pointer underline text-[#986c55]' onClick={handleBack}>
                        ...
                        <div className='w-full h-full flex flex-col'>
                            <div className='w-full px-6 py-2 h-[48%] lg:h-[58%] flex flex-col gap-3 overflow-y-scroll scrollbar-hide'>
                                {cart.cartItems?.map(cartItem => (
                                    <div className='w-full p-1 px-2 rounded-lg bg-[#f5f5f5] dark:bg-[#0e0e10] flex items-center gap-2' key={cartItem._id}>
                                        ...
                                        <div className='flex flex-col gap-2'>
                                            ...
                                            <p className='text-xs cursor-pointer flex items-center gap-1' onClick={() => handleRemoveFromCart(cartItem)}>Remove <AiOutlineClose/></p>
                                        </div>
                            ...
                            <div className='w-full gap-4 bg-[#f5f5f5] dark:bg-[#0e0e10] flex flex-col items-center justify-evenly px-8 py-2 rounded-md'>
                                ...
                                <div className='w-full p-2 rounded-full bg-[#986c55] text-lg my-2 hover:shadow-lg hover:scale-105 ease-in duration-300 transition-all cursor-pointer text-center'>Proceed to Checkout</div>
                            </div>
                        </div>
                    )}
